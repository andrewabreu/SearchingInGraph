...
class PathCostQueue(FIFOQueue):

    def extend(self, items):
        self.A.extend(items)
        #self.A.sort(cmp=cmp_path_cost)
        self.A.sort(cmp=lambda x,y: cmp(x.path_cost,y.path_cost))
...
#     def extend(self, items):
#         self.A.extend(items)
#         self.A.sort(cmp=cmp_path_cost, reverse=True)
#
#     def pop(self):
#         return self.A.pop()


def cmp_path_cost(x, y):
    return cmp(x.path_cost, y.path_cost)
