...
nexpandnodes = 0
def set_nexpandnodes(n):
    global nexpandnodes
    nexpandnodes = n

def get_nexpandnodes():
    return nexpandnodes

def graph_search(problem, fringe):
    count = 0
    set_nexpandnodes(count)
    """Search through the successors of a problem to find a goal.
    The argument fringe should be an empty queue.
...
def branch_and_bound_graph_search(problem):
    return graph_search(problem, PathCostQueue())


def branch_and_bound_graph_search2(problem):
    return graph_search(problem, PriorityQueue(min, f=lambda n: n.path_cost))


def branch_and_bound_with_underestimation_graph_search(problem, h=None):
    return graph_search(problem, PriorityQueue(min, f=lambda n: n.path_cost + h(n)))
...
# Busqueda de ramificacion y acotacion con subestimacion
...
# -*- coding: utf-8 -*-
# Falta la comprobacion de que la heuristica sea consistente y admisible. Si es consistente es admisible.
# Falta programacion dinamica
# A la espera de la clase de laboratorio
